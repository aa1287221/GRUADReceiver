...
class PickleDataLoad(object):
    def __init__(self, data_type, filename, augment_test_data=True):
        ...
        self.trainData, self.trainLabel = self.preprocessing(
            Path('dataset', data_type, 'labeled', 'train', filename), train=True)
        self.testData, self.testLabel = self.preprocessing(
            Path('dataset', data_type, 'labeled', 'test', filename), train=False)
    ...
    def augmentation(self, data, label, noise_ratio=0.05, noise_interval=0.0005, max_length=100000):
        ...
        for i in np.arange(0, noise_ratio, noise_interval):
            ...
            augmentedData = torch.cat(
                [augmentedData, data + scaled_noiseSeq], dim=0)
    ...
    def batchify(self, args, data, bsz):
        ...
        # print(trimmed_data.size())
        batched_data = trimmed_data.contiguous().view(
            bsz, -1, trimmed_data.size(-1)).transpose(0, 1)
