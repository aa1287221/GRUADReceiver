...
total_BER_noBlank = float(checkpoint[7])
total_BER_Known_Blank = float(checkpoint[8])
total_BER = float(checkpoint[9])
...
def anomaly_detection():
    ...
    for channel_idx in range(nfeatures):
        ...
        precision, recall, f_beta, error_point, accuracy, threshold, τ = get_precision_recall(
            mean, cov, sorted_error, args, score, num_samples=1000, beta=args.beta, label=TimeseriesData.testLabel.to(
                args.device))
...
for x in range(total_epochs - valid_epochs):
    ...
    OFDM_RX_noCP_noBlank = removeCP(OFDM_RX)
    OFDM_demod_noBlank = DFT(OFDM_RX_noCP_noBlank)
    equalized_Hest_noBlank = equalize(OFDM_demod_noBlank, Hest)
    QAM_est_noBlank = get_payload(equalized_Hest_noBlank)
    PS_est_noBlank, hardDecision_noBlank = Demapping(QAM_est_noBlank)
    bits_est_noBlank = PS(PS_est_noBlank)
    BER_noBlank = np.sum(abs(bits - bits_est_noBlank)) / len(bits)

    Known_OFDM_RX = np.copy(OFDM_RX)
    Unknown_OFDM_RX = np.copy(OFDM_RX)

    for Known_impulse in datacheck:
        Known_OFDM_RX[int(Known_impulse)-1] = 0
    OFDM_RX_noCP_Known_Blank = removeCP(Known_OFDM_RX)
    OFDM_demod_Known_Blank = DFT(OFDM_RX_noCP_Known_Blank)
    equalized_Hest_Known_Blank = equalize(OFDM_demod_Known_Blank, Hest)
    QAM_est_Known_Blank = get_payload(equalized_Hest_Known_Blank)
    PS_est_Known_Blank, hardDecision_Known_Blank = Demapping(
        QAM_est_Known_Blank)
    bits_est_Known_Blank = PS(PS_est_Known_Blank)
    BER_Known_Blank = np.sum(abs(bits - bits_est_Known_Blank)) / len(bits)
    ...
    # channelResponse = np.pad(channelResponse, (0, 1008))
    # OFDM_RX_DIAGONAL_MATRIX = channelResponse
    # for i in range(1, 1024, 1):
    #     OFDM_RX_CHANNEL = np.roll(channelResponse, i)
    #     OFDM_RX_DIAGONAL_MATRIX = np.vstack((
    #         OFDM_RX_DIAGONAL_MATRIX, OFDM_RX_CHANNEL))
    # u, s, vh = LA.svd(OFDM_RX_DIAGONAL_MATRIX)
    ...
    total_BER_noBlank += BER_noBlank
    total_BER_Known_Blank += BER_Known_Blank
    total_BER += BER
    valid_epochs += 1
    ...
    avg_BER_noBlank = (total_BER_noBlank / valid_epochs)
    avg_BER_Known_Blank = (total_BER_Known_Blank / valid_epochs)
    ...
    if valid_epochs % 1 == 0:
        ...
        print('Ac:', accuracy, ' F-beta:',
              fbeta, ' Pr:', precision, ' Rc:', recall, ' NoBlankBER', BER_noBlank, ' KnownBlankBER', BER_Known_Blank, ' BER:', BER)
        print('-' * 120)
        print(
            'epoch '
            + str(valid_epochs)
...
if valid_epochs == total_epochs:
    np.savetxt('resultSNR35.txt', np.reshape(
