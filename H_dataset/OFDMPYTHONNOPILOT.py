...
SNRdb = 70  # signal to noise-ratio in dB at the receiver
...
def channel(signal, INCounter):
    prob = np.random.uniform(0.001, 0.01)  # prob
...
for x in range(total_epochs - valid_epochs):
    ...
    OFDM_TX = OFDM_withCP
    OFDM_RX, INCounter = channel(OFDM_TX, INCounter)
    ...
    if valid_epochs % 100 == 0:
        print('Epochs :', valid_epochs, '\navg.BER =',
              avg_BER, '\nINProb =', (INCounter*100)/(1055*valid_epochs), '%')
    result.extend(['BER', avg_BER])
    if valid_epochs == total_epochs:
        np.savetxt('resultSNR70.txt', np.reshape(
